...
startChallengeBtn.onclick=()=>{level1.hidden=true;level2.hidden=false;GameAudio.stopExplorer();GameExplorer.hideHighlight();document.querySelectorAll('.connector-line').forEach(line => line.classList.remove('active'));document.querySelectorAll('.part-btn').forEach(button => button.classList.remove('active'));buildChallenge()};
...
startExamBtn.onclick=()=>{leaveChallenge();level2.hidden=true;level3.hidden=false;GameAudio.stopExplorer();GameAudio.stopChallenge();GameAudio.stopChallengeCompletion();GameExplorer.hideHighlight();hideChallengeHighlight();qi=0;score=0;loadQuestion()};
